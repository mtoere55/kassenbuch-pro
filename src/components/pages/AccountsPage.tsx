...
export function AccountsPage() {
  ...
  function prepareBookkeeping() {
    ...
    if (!providerCount) {
      setError("Bitte zuerst einen Zahlungsdienstleister-CSV über Universal Beleg Import importieren.");
      return;
    }
  ...
  const providerReview = provider.filter((item) => item.bookkeepingStatus === "booked" || item.status === "needsReview" || item.status === "new").length;
  ...
  return <div>
    <PageHeader
      title="Bank & Zahlungsabgleich"
      subtitle="Vendor-neutral: Alle Kontoauszüge, Zahlungsdienstleister-CSV und Monatsberichte werden über Universal Beleg Import hochgeladen und hier geprüft."
      actions={<div className="document-actions"><Button variant="secondary" onClick={reconcile}>Automatisch abgleichen</Button><Button onClick={prepareBookkeeping}>Zahlungsdienstleister buchen</Button></div>}
    />
    {message ? <div className="alert alert-success">{message}</div> : null}
    {error ? <div className="alert alert-danger">{error}</div> : null}

    <Card>
      <div className="card-heading"><div><h2>Ein Import für alle Nutzer</h2><p>Neue Dateien bitte über <strong>Universal Beleg Import</strong> hochladen. Das Programm erkennt PDF, Foto, CSV, TXT, Tagesabschluss, Eingangsrechnung, Kontoauszug und Zahlungsdienstleister-Export. Spezialformate werden nur noch im Hintergrund als Erkennungsregeln genutzt.</p></div></div>
    </Card>

    <div className="stat-grid">
      <StatCard label="Bankbewegungen" value={String(bankCount)} detail={`${bankBooked} gebucht · ${bankReview} prüfen`} />
      <StatCard label="Zahlungsdienstleister" value={String(providerCount)} tone="blue" detail={`${providerInternal} interne Umbuchungen`} />
      <StatCard label="Gebucht" value={String(providerBooked)} tone="positive" detail={`${providerReviewed} geprüft`} />
      <StatCard label="Gebühren" value={formatCurrency(providerFees)} tone="negative" detail={`${providerMatched} zugeordnet`} />
    </div>
    ...
    <Card>
      ...
      {sortedTransactions.length === 0 ? (
        <EmptyState icon="accounts" title="Noch keine Kontobewegungen" text="Lade Kontoauszug oder Zahlungsdienstleister-CSV über Universal Beleg Import hoch." />
      ) : (
        <div className="table-wrap">
          <table className="data-table">
            ...
            <tbody>{sortedTransactions.map((item) => <TransactionRow key={item.id} item={item} document={state.documents.find((document) => document.id === item.matchedDocumentId)} onReview={() => setSelectedId(item.id)} />)}</tbody>
          </table>
        </div>
      )}
    </Card>
...
function TransactionRow({ item, document, onReview }: { item: ImportedTransaction; document?: BusinessDocument; onReview: () => void }) {
  ...
  return <tr>
    ...
    <td><Badge tone={item.accountType === "paypal" ? "info" : "neutral"}>{item.accountType === "paypal" ? "Dienstleister" : "Bank"}</Badge></td>
