...
interface Props { entry?: LedgerEntry; onClose: () => void; onSaved: (message: string) => void; }
interface EntryDraft { date: string; direction: LedgerDirection; paymentMethod: PaymentMethod; bookingAccountCode: string; description: string; amount: string; taxRate: number; documentNumber: string; note: string; }
...
export function LedgerEntryEditModal({ entry, onClose, onSaved }: Props) {
  ...
  function patch<K extends keyof EntryDraft>(key: K, value: EntryDraft[K]) { setDraft((current) => ({ ...current, [key]: value })); }
  ...
  function save() {
    ...
    try {
      ...
      const nextTaxMode: TaxMode = isDifferentialSale ? "differential" : draft.taxRate ? entry.taxMode === "differential" ? "differential" : "standard19" : "taxFree";
      ...
      const nextEntry: LedgerEntry = {
        ...entry, date: draft.date, direction: draft.direction, amount, paymentMethod: draft.paymentMethod,
        description: draft.description.trim() || bookingLabel, category: `${draft.bookingAccountCode} · ${bookingLabel}`,
        taxAmount: updatedTaxAmount, taxRate: draft.taxRate, taxMode: nextTaxMode,
  ...
  return <Modal open={Boolean(entry)} title="Buchung bearbeiten" onClose={onClose} wide footer={<Fragment>{entry.documentId ? null : <Button variant="danger" onClick={deleteEntry}>Buchung löschen</Button>}<Button variant="secondary" icon="print" onClick={printCard}>Drucken</Button><Button variant="secondary" onClick={onClose}>Schließen</Button><Button onClick={save}>Änderung speichern</Button></Fragment>}>
    ...
    <div ref={printRef} className="print-only entry-print-card" data-print-kind="booking-card"><div className="document-head"><div><div className="document-brand">Kassenbuch Pro</div><div>{state.settings.businessName}</div><div>{state.settings.ownerName}</div><div>{state.settings.street}</div><div>{state.settings.postalCode} {state.settings.city}</div></div><div className="document-meta"><h1>Buchung</h1><div><span>Datum</span><strong>{formatDate(draft.date)}</strong></div><div><span>Beleg</span><strong>{draft.documentNumber || relatedDocument?.documentNumber || "-"}</strong></div></div></div><table className="document-table"><tbody><tr><th>Kassenkonto</th><td>{cashAccountTitle(entry)}</td></tr><tr><th>Buchungskonto</th><td>{draft.bookingAccountCode} · {bookingAccount?.label || accountLabelFromEntry(entry)}</td></tr><tr><th>Text</th><td>{draft.description || "-"}</td></tr><tr><th>Betrag</th><td>{formatCurrency(amount)}</td></tr>{isDifferentialSale ? <tr><th>§25a Marge</th><td>{formatCurrency(Math.max(0, amount - (relatedDevice?.purchasePrice || 0)))}</td></tr> : null}<tr><th>MwSt.</th><td>{draft.taxRate ? `${draft.taxRate} % / ${formatCurrency(taxAmount)}` : "0 %"}</td></tr><tr><th>Netto</th><td>{formatCurrency(netAmount)}</td></tr><tr><th>Kassenwirkung</th><td>{cashChange >= 0 ? "+" : "−"}{formatCurrency(Math.abs(cashChange))}</td></tr></tbody></table>{draft.note ? <p className="tax-note">Notiz: {draft.note}</p> : null}</div>
  </Modal>;
}

function createDraft(entry?: LedgerEntry, documentNumber?: string): EntryDraft { return { date: entry?.date || "", direction: entry?.direction || "income", paymentMethod: entry?.paymentMethod || "cash", bookingAccountCode: entry ? bookingAccountCode(entry) : "8400", description: entry?.description || "", amount: entry ? moneyInput(entry.amount) : "", taxRate: entry?.taxRate ?? 0, documentNumber: entry?.documentNumber || documentNumber || "", note: entry?.note || "" }; }
function bookingAccountCode(entry: LedgerEntry): string { if (entry.direction === "transfer" && entry.counterAccountCode) return entry.counterAccountCode; if (entry.sourceId?.startsWith("unitel-sales:") && entry.counterAccountCode) return entry.counterAccountCode; if (entry.sourceId?.startsWith("prifoto-sales:") && entry.counterAccountCode) return entry.counterAccountCode; if (entry.accountCode) return entry.accountCode; const match = entry.category.match(/^(\d{4})/); return match?.[1] || (entry.taxMode === "differential" && entry.direction === "income" ? "8336" : entry.direction === "expense" ? "3290" : "0000"); }
function accountLabelFromEntry(entry: LedgerEntry): string { const code = bookingAccountCode(entry); return getBookingCategory(code)?.label || entry.category.split("·").slice(1).join("·").trim() || entry.category; }
function cashAccountTitle(entry: LedgerEntry): string { if (entry.paymentMethod === "card") return "01360 Geldtransit und Karte"; if (entry.paymentMethod === "bank") return "01200 Bank"; if (entry.paymentMethod === "paypal") return "01370 PayPal"; return "01000 Kasse"; }
