...
export function LedgerEntryEditModal({ entry, onClose, onSaved }: Props) {
  ...
  const [draft, setDraft] = useState<EntryDraft>(() => createDraft(entry));
  ...
  const taxAmount = includedTax(amount, draft.taxRate);
  ...
  function save() {
    ...
    try {
      ...
      const updatedTaxAmount = includedTax(amount, draft.taxRate);
      const nextTaxMode: TaxMode = draft.taxRate
      ...
      replaceState({
        ...
        documents: state.documents.map((document) => {
          ...
          return {
            ...
            taxMode: nextTaxMode,
            paymentMethod: draft.paymentMethod,
            documentNumber: draft.documentNumber.trim() || document.documentNumber,
          };
        }),
      });
    ...
    >
      <div className="screen-only">
        <div className="entry-card-head">
          <div>
            <small>Buchen / Bearbeiten</small>
            <strong>{formatDate(entry.date)}</strong>
          </div>
          <div>
            <small>Konto</small>
            <strong>{cashAccountTitle(entry)}</strong>
          </div>
          <div>
            <small>Kassenwirkung</small>
            <strong className={cashChange < 0 ? "money-negative" : "money-positive"}>{cashChange >= 0 ? "+" : "−"}{formatCurrency(Math.abs(cashChange))}</strong>
          </div>
        </div>
        {error ? <div className="alert alert-danger">{error}</div> : null}
        {entry.source !== "manual" ? (
          <div className="alert alert-info">
            Diese Buchung kommt aus einem Import ({entry.source}). Änderungen sind möglich, aber der ursprüngliche Importbeleg bleibt zur Kontrolle erhalten.
          </div>
        ) : null}
...
function createDraft(entry?: LedgerEntry): EntryDraft {
  return {
    date: entry?.date || "",
