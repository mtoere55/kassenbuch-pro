...
import { buildReviewAccountOptions, isKasImportEntry, isUnresolvedKasEntry, ledgerSourceLabel } from "@/lib/kas-review";
...
export function LedgerPage() {
  ...
  const sorted = useMemo(() => [...state.ledger].sort((a, b) => `${a.date}|${a.createdAt}`.localeCompare(`${b.date}|${b.createdAt}`)), [state.ledger]);
  ...
  const monthRows = sorted.filter((entry) => entry.date >= start && entry.date < end).reduce<Array<{ entry: LedgerEntry; balance: number }>>((rows, entry) => { const previous = rows.length ? rows[rows.length - 1].balance : opening; rows.push({ entry, balance: previous + entryCashEffect(entry) }); return rows; }, []);
  const rows = monthRows.filter(({ entry }) => {
    ...
    return (payment === "all" || entry.paymentMethod === payment) && (direction === "all" || entry.direction === direction) && (!unresolvedOnly || isUnresolvedKasEntry(entry)) && (!query || text.includes(query.toLowerCase()));
  });
  ...
  function printReport(mode: PrintMode) { setPrintMode(mode); window.setTimeout(() => printFirst(".ledger-print-source", `Kassenbuch ${monthLabel(month)}`), 60); }
  function exportCsv() {
    const header = ["Datum", "Beleg", "Einnahmen", "Ausgaben", "MwSt", "USt", "VSt", "Ein Netto", "Aus Netto", "Saldo", "Konto", "Text"];
    const csvRows = [header, ...rows.map(({ entry, balance }) => { const effect = entryCashEffect(entry); const isIncome = entry.direction === "income"; const isExpense = entry.direction === "expense"; return [entry.date, entry.documentNumber || "", effect > 0 ? euroNumber(effect) : "", effect < 0 ? euroNumber(Math.abs(effect)) : "", entry.taxRate ? `${entry.taxRate} %` : "", isIncome && entry.taxAmount ? euroNumber(entry.taxAmount) : "", isExpense && entry.taxAmount ? euroNumber(entry.taxAmount) : "", isIncome ? euroNumber(entry.netAmount ?? entry.amount - entry.taxAmount) : "", isExpense ? euroNumber(entry.netAmount ?? entry.amount - entry.taxAmount) : "", euroNumber(balance), displayAccountCode(entry), entry.description]; })];
    const csv = csvRows.map((line) => line.map(quoteCsv).join(";")).join("\n");
    const blob = new Blob([`\uFEFF${csv}`], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `Kassenbuch_${month}.csv`; link.click(); URL.revokeObjectURL(url);
  }

  return <div>
    <PageHeader title="Kassenbuch" subtitle="Monatliches Kassenkonto mit laufendem Saldo, sauberem Druck, CSV-Ausgabe und anklickbarer Buchungskarte." actions={<div className="cashbook-actions"><Button icon="plus" onClick={() => startBooking("income")}>Neue Buchung</Button><Button variant="secondary" icon="print" onClick={() => printReport("normal")}>Drucken</Button><Button variant="secondary" icon="print" onClick={() => printReport("receipts")}>Drucken mit Belegen</Button><Button variant="secondary" icon="download" onClick={exportCsv}>CSV-Datei</Button></div>} />
