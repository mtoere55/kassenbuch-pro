...
export function LedgerPage() {
  ...
  const rows = monthRows.filter(({ entry }) => {
    ...
    return matchesCashbookPaymentFilter(entry, payment) &&
  ...
  function printReport(mode: PrintMode) {
    setPrintMode(mode);
    window.setTimeout(() => printFirst(".ledger-print-source", `Kassenbuch ${monthLabel(month)}`), 60);
  }
  ...
  return <div>
    <PageHeader
      ...
      actions={<div className="cashbook-actions">
        <Button icon="plus" onClick={() => startBooking("income")}>Neue Buchung</Button>
        <Button variant="secondary" icon="print" onClick={() => printReport("normal")}>Drucken</Button>
        <Button variant="secondary" icon="print" onClick={() => printReport("receipts")}>Drucken mit Belegen</Button>
        <Button variant="secondary" icon="download" onClick={exportCsv}>CSV-Datei</Button>
      </div>}
    />
    ...
    <div className="view-tabs">
      <button className={view === "book" ? "active" : ""} onClick={() => setView("book")}>Kassenbuch</button>
      <button className={view === "accounts" ? "active" : ""} onClick={() => setView("accounts")}>Kontenplan</button>
    </div>
    {view === "accounts" ? <AccountPlan ledger={state.ledger} /> : <>
      <div className="stat-grid">
        <StatCard label="Anfangsbestand" value={formatCurrency(opening)} detail={monthLabel(month)} />
        <StatCard label="Kasseneinnahmen" value={formatCurrency(cashIncome)} tone="positive" />
        <StatCard label="Kassenausgaben" value={formatCurrency(cashExpense)} tone="negative" />
        <StatCard label="Kassenendbestand" value={formatCurrency(ending)} tone="blue" detail={`USt ${formatCurrency(outputTax)} · VSt ${formatCurrency(inputTax)}`} />
      </div>
      <Card>
        <div className="booking-shortcuts">
          <Button variant="secondary" onClick={() => startBooking("income")}>+ Einnahme</Button>
          <Button variant="secondary" onClick={() => startBooking("expense")}>- Ausgabe</Button>
          <Button variant="secondary" onClick={() => startBooking("transfer")}>Umbuchung</Button>
          <Button variant="secondary" onClick={() => startBooking("private")}>Privat</Button>
        ...
        <div className="toolbar ledger-toolbar">
          ...
          <Select value={direction} onChange={(event) => setDirection(event.target.value as typeof direction)}>
            <option value="all">Alle Vorgange</option>
            <option value="income">Einnahmen</option>
            <option value="expense">Ausgaben</option>
            <option value="transfer">Umbuchung / Fremdgeld</option>
          </Select>
          <Select value={payment} onChange={(event) => setPayment(event.target.value as typeof payment)}>
            <option value="cash">Kassenwirkung inkl. Umbuchungen</option>
        ...
        {rows.length ? <div className="table-wrap">
          <table className="data-table cashbook-table">
            <thead><tr><th>Datum</th><th>Beleg</th><th>Einnahmen</th><th>Ausgaben</th><th>MwSt.</th><th>USt</th><th>VSt</th><th>Ein. Netto</th><th>Aus. Netto</th><th>Saldo</th><th>Konto</th><th>Text</th></tr></thead>
            <tbody>{rows.map(({ entry, balance }) => {
              ...
              return <tr
                key={entry.id}
                className="cashbook-row"
                tabIndex={0}
                onClick={() => setSelectedEntryId(entry.id)}
                onKeyDown={(event) => { if (event.key === "Enter") setSelectedEntryId(entry.id); }}
              >
                <td>{formatDate(entry.date)}</td>
                <td><strong>{entry.documentNumber || "-"}</strong>{entry.attachmentDataUrl ? <small>Beleg gespeichert</small> : null}</td>
                <td className="money-positive">{effect > 0 ? formatCurrency(effect) : ""}</td>
                <td className="money-negative">{effect < 0 ? formatCurrency(Math.abs(effect)) : ""}</td>
                ...
                <td>{isIncome ? formatCurrency(entry.netAmount ?? entry.amount - entry.taxAmount) : ""}</td>
                <td>{isExpense ? formatCurrency(entry.netAmount ?? entry.amount - entry.taxAmount) : ""}</td>
                <td><strong>{formatCurrency(balance)}</strong></td>
                <td><strong>{displayAccountCode(entry)}</strong><small>{displayAccountLabel(entry)}</small></td>
                <td>
                  <strong>{entry.description}</strong>
                  <small>{entry.direction === "transfer" ? transferLabel(entry) : `${paymentLabel(entry.paymentMethod)} · ${ledgerSourceLabel(entry)}`}</small>
                  {isKasImportEntry(entry) ? <span className="document-actions"><Badge tone={unresolved ? "warning" : "success"}>{unresolved ? "Ungeklart" : "Gepruft"}</Badge></span> : null}
                </td>
              </tr>;
            })}</tbody>
            <tfoot><tr><td colSpan={2}><strong>Gesamt {monthLabel(month)}</strong></td><td>{formatCurrency(cashIncome)}</td><td>{formatCurrency(cashExpense)}</td><td /><td>{formatCurrency(outputTax)}</td><td>{formatCurrency(inputTax)}</td><td>{formatCurrency(tradeIncome - outputTax)}</td><td>{formatCurrency(tradeExpense - inputTax)}</td><td><strong>{formatCurrency(ending)}</strong></td><td colSpan={2} /></tr></tfoot>
          </table>
        </div> : <EmptyState icon="ledger" title="Keine Buchungen gefunden" text="Automatische, importierte und manuelle Kassenvorgange erscheinen hier." action={<Button onClick={() => startBooking("income")}>Buchung erfassen</Button>} />}
      </Card>
    </>}
    ...
    <LedgerEntryEditModal key={selectedEntry?.id || "no-entry"} entry={selectedEntry} onClose={() => setSelectedEntryId(undefined)} onSaved={setNotice} />
  </div>;
}

function LedgerPrintDocument({ rows, month, opening, ending, cashIncome, cashExpense, outputTax, inputTax, settings, includeReceipts }: {
  ...
  return <div className="ledger-print-source" data-print-kind="ledger-report">
    <div className="ledger-print-document">
      <header className="ledger-print-head">
        <div><h1>Kassenbuch</h1><p>{monthLabel(month)}</p></div>
        <div><strong>{settings.businessName}</strong><span>{settings.ownerName}</span><span>{settings.street}</span><span>{settings.postalCode} {settings.city}</span>{settings.taxNumber ? <span>Steuernr. {settings.taxNumber}</span> : null}</div>
      </header>
...
function AccountPlan({ ledger }: { ledger: LedgerEntry[] }) {
  const accounts = buildReviewAccountOptions(ledger);
  return <Card>
    <div className="card-heading"><div><h2>Kontenplan</h2><p>Voreingestellte und aus KAS-Backups uebernommene Konten.</p></div></div>
    <div className="table-wrap"><table className="data-table"><thead><tr><th>Nummer</th><th>Bezeichnung</th><th>Typ</th><th>MwSt.</th></tr></thead><tbody>{accounts.map((item) => <tr key={item.code}><td><strong>{item.code}</strong></td><td>{item.label}</td><td><Badge>{item.side === "in" ? "Einnahmen" : item.side === "out" ? "Ausgaben" : "Neutral"}</Badge></td><td>{item.vat ? `${item.vat} %` : "-"}</td></tr>)}</tbody></table></div>
  </Card>;
}

function total(entries: Array<{ direction: LedgerDirection; amount: number }>, direction: LedgerDirection) {
  return entries.filter((entry) => entry.direction === direction).reduce((sum, entry) => sum + entry.amount, 0);
}

function paymentLabel(method: PaymentMethod) {
  return ({ cash: "Bar", card: "Karte", bank: "Bank", paypal: "PayPal" } as const)[method];
}

function transferLabel(entry: LedgerEntry) {
  if (entry.source === "unitelImport" && entry.sourceId?.startsWith("unitel-sales:")) return `UniTel Fremdgeld · ${paymentLabel(entry.paymentMethod)}`;
  if (entry.source === "prifotoImport" && entry.sourceId?.startsWith("prifoto-sales:")) return `Prifoto Clearing · ${paymentLabel(entry.paymentMethod)}`;
  return entry.manualKind === "private"
    ? entry.cashChange && entry.cashChange > 0 ? "Privateinlage" : "Privatentnahme"
    : entry.cashChange && entry.cashChange > 0 ? "Bank an Kasse" : "Kasse an Bank";
}
...
function displayAccountCode(entry: LedgerEntry): string {
  if ((entry.direction === "transfer" || entry.sourceId?.startsWith("unitel-sales:") || entry.sourceId?.startsWith("prifoto-sales:")) && entry.counterAccountCode) return entry.counterAccountCode;
  return entry.accountCode || entry.category.match(/^(\d{4})/)?.[1] || "-";
}

function displayAccountLabel(entry: LedgerEntry): string {
  const code = displayAccountCode(entry);
  return getBookingCategory(code)?.label || entry.category.split("·").slice(1).join("·").trim() || entry.category;
}

function euroNumber(value: number): string {
