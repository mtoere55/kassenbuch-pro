...
export function LedgerPage() {
  ...
  const monthRows = sorted.filter((entry) => entry.date >= start && entry.date < end).reduce<Array<{ entry: LedgerEntry; balance: number }>>((rows, entry) => {
  ...
  function exportCsv() {
    ...
    const csvRows = [header, ...rows.map(({ entry, balance }) => {
      ...
      return [entry.date, entry.documentNumber || "", effect > 0 ? euroNumber(effect) : "", effect < 0 ? euroNumber(Math.abs(effect)) : "", entry.taxRate ? `${entry.taxRate} %` : "", isIncome && entry.taxAmount ? euroNumber(entry.taxAmount) : "", isExpense && entry.taxAmount ? euroNumber(entry.taxAmount) : "", isIncome ? euroNumber(entry.netAmount ?? entry.amount - entry.taxAmount) : "", isExpense ? euroNumber(entry.netAmount ?? entry.amount - entry.taxAmount) : "", euroNumber(balance), displayAccountCode(entry), entry.description];
    })];
