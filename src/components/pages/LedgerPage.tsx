  ...
  return <div>
    ...
    {view === "accounts" ? <AccountPlan ledger={state.ledger} /> : <>
      ...
      <Card>
        ...
        {rows.length ? <div className="table-wrap"><table className="data-table cashbook-table"><thead><tr><th>Datum</th><th>Beleg</th><th>Einnahmen</th><th>Ausgaben</th><th>MwSt.</th><th>USt</th><th>VSt</th><th>Ein. Netto</th><th>Aus. Netto</th><th>Saldo</th><th>Konto</th><th>Text</th></tr></thead><tbody>{rows.map(({ entry, balance }) => { const isIncome = entry.direction === "income"; const isExpense = entry.direction === "expense"; const effect = entryCashEffect(entry); const unresolved = isUnresolvedKasEntry(entry); return <tr key={entry.id}><td>{formatDate(entry.date)}</td><td><strong>{entry.documentNumber || "-"}</strong>{entry.attachmentDataUrl ? <small>Beleg gespeichert</small> : null}</td><td className="money-positive">{effect > 0 ? formatCurrency(effect) : ""}</td><td className="money-negative">{effect < 0 ? formatCurrency(Math.abs(effect)) : ""}</td><td>{entry.taxMode === "differential" ? "25a" : entry.taxRate ? `${entry.taxRate} %` : "-"}</td><td>{isIncome && entry.taxAmount ? formatCurrency(entry.taxAmount) : ""}</td><td>{isExpense && entry.taxAmount ? formatCurrency(entry.taxAmount) : ""}</td><td>{isIncome ? formatCurrency(entry.netAmount ?? entry.amount - entry.taxAmount) : ""}</td><td>{isExpense ? formatCurrency(entry.netAmount ?? entry.amount - entry.taxAmount) : ""}</td><td><strong>{formatCurrency(balance)}</strong></td><td><strong>{entry.accountCode || "-"}</strong><small>{entry.category.split("·").slice(1).join("·").trim() || entry.category}</small></td><td><strong>{entry.description}</strong><small>{entry.direction === "transfer" ? transferLabel(entry) : `${paymentLabel(entry.paymentMethod)} · ${ledgerSourceLabel(entry)}`}</small>{isKasImportEntry(entry) ? <span className="document-actions"><Badge tone={unresolved ? "warning" : "success"}>{unresolved ? "Ungeklart" : "Gepruft"}</Badge><Button variant="secondary" onClick={() => setSelectedKasId(entry.id)}>Prufen</Button></span> : null}</td></tr>; })}</tbody><tfoot><tr><td colSpan={2}><strong>Gesamt {monthLabel(month)}</strong></td><td>{formatCurrency(cashIncome)}</td><td>{formatCurrency(cashExpense)}</td><td /><td>{formatCurrency(outputTax)}</td><td>{formatCurrency(inputTax)}</td><td>{formatCurrency(tradeIncome - outputTax)}</td><td>{formatCurrency(tradeExpense - inputTax)}</td><td><strong>{formatCurrency(ending)}</strong></td><td colSpan={2} /></tr></tfoot></table></div> : <EmptyState icon="ledger" title="Keine Buchungen gefunden" text="Automatische, importierte und manuelle Kassenvorgange erscheinen hier." action={<Button onClick={() => startBooking("income")}>Buchung erfassen</Button>} />}
      </Card>
    </>}
