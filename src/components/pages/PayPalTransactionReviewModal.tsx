...
function PayPalReviewForm({
  ...
  const [taxRate, setTaxRate] = useState<ReviewTaxRate>(direction === "income" ? 19 : 0);
  const [description, setDescription] = useState(transaction.description || transaction.counterparty || "Zahlungsdienstleister-Umsatz");
  ...
  function save() {
    try {
      const nextState = reviewPayPalTransaction(state, transaction.id, {
        direction,
        accountCode,
        paymentMethod,
        taxRate,
      ...
      onSaved(`Zahlungsdienstleister-Zahlung wurde geprüft und auf ${accountCode} gebucht.`);
      onClose();
    } catch (cause) {
      onSaved(cause instanceof Error ? cause.message : "Zahlung konnte nicht geprüft werden.");
    }
  }
