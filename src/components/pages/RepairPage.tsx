...
import { printDocumentView, DocumentView } from "../documents/DocumentView";
...
export function RepairPage() {
  ...
  const filteredRepairs = useMemo(() => {
    return repairs.filter((repair) => statusFilter === "all" || repair.status === statusFilter);
  }, [repairs, statusFilter]);
  const openCount = repairs.filter((repair) => !["paid", "cancelled"].includes(repair.status)).length;
  const monthRevenue = state.ledger
    .filter((entry) => entry.source === "repair" && entry.date.slice(0, 7) === todayIso().slice(0, 7))
  ...
  function createRepair() {
    ...
    if (!draft.brand.trim() || !draft.model.trim()) {
      setError("Bitte Marke und Modell eintragen.");
      return;
    }
    if (!draft.issue.trim()) {
      setError("Bitte Fehlerbeschreibung / Kundenauftrag eintragen.");
      return;
    }
    if (draft.documentType !== "estimate" && price <= 0) {
      setError("Für Rechnung oder Quittung bitte einen Betrag größer 0,00 € eintragen.");
      return;
    }

    const createdAt = new Date().toISOString();
    const existingRepairs = repairs.map((repair) => repair.repairNumber);
    const repairNumber = nextSequence("REP", existingRepairs, new Date(`${draft.date}T12:00:00`));
    ...
    const status: RepairStatus = draft.documentType === "estimate" ? "estimate" : "paid";
    ...
    replaceState({
      ...
      customers: state.customers.map((customer) => {
        if (customer.id !== customerId || customer.roles.includes("repair")) return customer;
        return { ...customer, roles: [...customer.roles, "repair"] };
      }),
    });
  ...
  return (
    <div>
      ...
      <div className="stat-grid compact">
        <StatCard label="Offene Reparaturen" value={String(openCount)} />
        <StatCard label="Abgeschlossen" value={String(repairs.filter((repair) => repair.status === "paid").length)} tone="positive" />
        <StatCard label="Monat Reparaturumsatz" value={formatCurrency(monthRevenue)} tone="blue" />
      </div>
      ...
      <div className="grid two-col">
        <Card>
          ...
          <div className="form-grid two">
            ...
            <Field label="Modell"><Input value={draft.model} onChange={(event) => patch("model", event.target.value)} placeholder="z.B. S26 Ultra" /></Field>
          ...
          <div className="form-stack booking-details">
            <Field label="Fehlerbeschreibung / Kundenauftrag"><textarea className="input textarea" value={draft.issue} onChange={(event) => patch("issue", event.target.value)} placeholder="Display defekt, lädt nicht, Wasserschaden..." /></Field>
            <Field label="Leistung / Reparaturtext"><textarea className="input textarea" value={draft.workDescription} onChange={(event) => patch("workDescription", event.target.value)} placeholder="Displaytausch inkl. Ersatzteil und Arbeit" /></Field>
          </div>
          <div className="form-grid two booking-details">
            <Field label="Dokument"><Select value={draft.documentType} onChange={(event) => patch("documentType", event.target.value as RepairDocumentType)}><option value="estimate">Kostenvoranschlag</option><option value="invoice">Rechnung</option><option value="receipt">Quittung</option></Select></Field>
            <Field label="Zahlungsart"><Select value={draft.paymentMethod} onChange={(event) => patch("paymentMethod", event.target.value as PaymentMethod)} disabled={draft.documentType === "estimate"}><option value="cash">Bar / Kasse</option><option value="card">Karte</option><option value="bank">Bank</option><option value="paypal">PayPal</option></Select></Field>
            <Field label="Preis brutto"><Input inputMode="decimal" value={draft.price} onChange={(event) => patch("price", event.target.value)} placeholder="0,00" /></Field>
            <Field label="Notiz intern"><Input value={draft.notes} onChange={(event) => patch("notes", event.target.value)} /></Field>
          </div>
...
function createDraft(): RepairDraft {
  return {
    customerId: "",
    date: todayIso(),
    brand: "",
...
function nextRepairDocumentNumber(type: RepairDocumentType, existing: string[], date: string): string {
  const prefix = type === "estimate" ? "KV" : type === "invoice" ? "RE" : "QU";
  return nextSequence(prefix, existing, new Date(`${date}T12:00:00`));
}
