...
import { createDeviceSale, taxModeLabel } from "@/lib/sale-workflow";
import { useKassenStore } from "@/lib/store";
import type { PaymentMethod, TaxMode } from "@/lib/types";
...
export function SalePage() {
  ...
  const [deviceId, setDeviceId] = useState(first?.id ?? "");
  const [customerId, setCustomerId] = useState("");
  const [date, setDate] = useState(todayIso());
  const [price, setPrice] = useState(String(first?.askingPrice ?? first?.purchasePrice ?? ""));
  const [paymentMethod, setPaymentMethod] = useState<PaymentMethod>("cash");
  const [documentType, setDocumentType] = useState<"invoice" | "receipt">("invoice");
  const [taxMode, setTaxMode] = useState<TaxMode>(first?.taxMode ?? "differential");
  ...
  function chooseDevice(id: string) {
    ...
    setTaxMode(selected?.taxMode ?? "differential");
  }
  ...
  function submit() {
    ...
    try {
      const result = createDeviceSale(state, {
      ...
      setTaxMode(next?.taxMode ?? "differential");
    } catch (cause) {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Gerät wählen, Steuerbehandlung bestätigen, Zahlung erfassen und Rechnung oder Quittung erstellen."
      />
      ...
      {available.length === 0 ? (
        ...
        <div className="workflow-grid">
            ...
            <div className="form-stack">
              ...
              {device ? (
                <div className="device-summary">
                  ...
                  <div className="summary-tags"><Badge tone="info">IMEI {device.imei1}</Badge><Badge>{device.condition}</Badge><Badge tone={device.taxMode === "differential" ? "warning" : "info"}>{device.taxMode === "differential" ? "Ankauf §25a" : device.taxMode === "standard19" ? "Ankauf 19 %" : "Ankauf 0 %"}</Badge></div>
          ...
          <Card>
            <div className="step-title"><span>2</span><div><h2>Zahlung, Steuer und Beleg</h2><p>Die Steuerart wird für diesen Verkauf ausdrücklich gespeichert.</p></div></div>
            <div className="form-grid two">
              ...
              <Field label="Steuerbehandlung Verkauf">
                <Select value={taxMode} onChange={(event) => setTaxMode(event.target.value as TaxMode)}>
                  <option value="differential">Differenzbesteuerung §25a UStG</option>
            ...
            {device && device.taxMode !== taxMode ? (
              <div className="alert alert-warning">
                Ankauf war „{taxModeLabel(device.taxMode)}“, Verkauf ist „{taxModeLabel(taxMode)}“. Bitte nur mit passendem Ankaufbeleg ändern.
              </div>
            ) : null}
            {taxMode === "differential" ? (
              <div className="alert alert-info">Auf Rechnung und Buchung wird „Differenzbesteuerung nach §25a UStG“ ausgewiesen. Die Umsatzsteuer wird nur aus der positiven Marge berechnet.</div>
            ) : null}
            {device && metrics ? (
              <div className="calculation-box">
                ...
                {taxMode === "differential" ? <div><span>Enthaltene Differenz-MwSt.</span><strong>{formatCurrency(metrics.differentialVat)}</strong></div> : null}
                {taxMode === "standard19" ? <div><span>Enthaltene MwSt. 19 %</span><strong>{formatCurrency(metrics.standardVat)}</strong></div> : null}
