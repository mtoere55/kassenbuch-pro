...
export function useScannerController() {
  ...
  async function recognizeSources(sources: Array<File | Blob>) {
    ...
    let activePage = 0; const totalPages = Math.max(sources.length, 1);
    const worker = await createWorker(["deu", "eng"], 1, { logger: (event) => { if (event.status === "recognizing text") setProgress(Math.round(((activePage + (event.progress || 0)) / totalPages) * 100)); } });
    try {
      await worker.setParameters({ preserve_interword_spaces: "1" });
      const parts: string[] = [];
      for (let index = 0; index < sources.length; index += 1) { activePage = index; const result = await worker.recognize(sources[index]); if (result.data.text.trim()) parts.push(result.data.text.trim()); }
      return parts.join("\n");
  ...
  async function saveSupplierInvoice(invoice: ParsedInvoice, dataUrl?: string) {
    ...
    const document: BusinessDocument = { id: documentId, documentNumber, type: "supplierInvoice", date, amount: amounts.gross, taxAmount: amounts.vat, taxMode: amounts.vat > 0 ? "standard19" : "taxFree", paymentMethod, status: invoicePaid ? "paid" : "open", originalFileName: file?.name, originalImageDataUrl: dataUrl, ocrText, metadata: { vendor, invoiceNumber: invoice.invoiceNumber || null, accountCode: account.code, accountLabel: account.label, duplicateKey, automaticallyBooked: true, universalImport: true }, createdAt };
    const paymentAccount = { cash: "1000", card: "1360", bank: "1200", paypal: "1370" }[paymentMethod];
    const ledgerEntry: LedgerEntry = { id: makeId("ledger"), date, direction: "expense", amount: amounts.gross, paymentMethod, description: `Eingangsrechnung ${vendor}`, category: `${account.code} · ${account.label}`, source: "scan", sourceId: documentId, documentId, taxAmount: amounts.vat, taxRate: amounts.vat > 0 ? 19 : 0, taxMode: amounts.vat > 0 ? "standard19" : "taxFree", reconciled: invoicePaid && (paymentMethod === "cash" || paymentMethod === "card"), accountCode: account.code, counterAccountCode: paymentAccount, documentNumber, cashChange: paymentMethod === "cash" && invoicePaid ? -amounts.gross : 0, netAmount: amounts.net, attachmentFileName: file?.name, attachmentDataUrl: dataUrl, createdAt };
  ...
  async function save() {
    ...
    try {
      ...
      if (!parsed) return;
      const dataUrl = await originalFileDataUrl(); const archiveNote = file && !dataUrl ? " Die Originaldatei ist für den lokalen Demo-Speicher zu groß; Dateiname und OCR-Text wurden gespeichert." : "";
      if (parsed.type === "zReport") { if (!parsed.gross && !parsed.cash && !parsed.card) throw new Error("Bitte Brutto, Bar oder Karte kontrollieren. Ohne Umsatzwert kann der Tagesabschluss nicht gespeichert werden."); addScannedZReport({ date: parsed.date || todayIso(), zNumber: parsed.zNumber, gross: parsed.gross || 0, net: parsed.net, vat: parsed.vat, cash: parsed.cash || 0, card: parsed.card || 0, salesCount: parsed.salesCount, difference: parsed.difference, imageDataUrl: dataUrl, fileName: file?.name, ocrText, bookSales }); setMessage((bookSales ? "Tagesabschluss archiviert und Umsatz gebucht." : "Tagesabschluss archiviert und nur zum Abgleich gespeichert.") + archiveNote); }
      else { await saveSupplierInvoice(parsed, dataUrl); const account = getSupplierAccount(accountCode); setMessage(`Beleg gespeichert und auf ${account.code} ${account.label} in die Buchhaltung übernommen.` + archiveNote); }
