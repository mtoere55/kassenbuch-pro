...
export function useScannerController() {
  ...
  async function saveSupplierInvoice(invoice: ParsedInvoice, dataUrl?: string) {
    ...
    if (duplicate) {
      throw new Error(`Diese Rechnung ist bereits als ${duplicate.documentNumber} gespeichert. Bitte die vorhandene Rechnung öffnen oder zuerst löschen.`);
    }

    const createdAt = new Date().toISOString();
    const documentId = makeId("document");
    const documentNumber = invoice.invoiceNumber?.trim() || nextSequence(
      "ER",
  ...
  async function save() {
    ...
    try {
      ...
      if (parsed.type === "zReport") {
        if (!parsed.gross && !parsed.cash && !parsed.card) {
          throw new Error("Bitte Brutto, Bar oder Karte kontrollieren. Ohne Umsatzwert kann der Tagesabschluss nicht gespeichert werden.");
        }
        addScannedZReport({
        ...
        const account = getSupplierAccount(accountCode);
        setMessage(`Eingangsrechnung gespeichert und automatisch auf ${account.code} ${account.label} in die Buchhaltung übernommen.` + archiveNote);
      }
    } catch (cause) {
  ...
  return {
    ...
    error,
    scanInfo,
    differenceWarning: parsed?.type === "zReport" && Boolean(parsed.difference),
    incompleteWarning: parsed ? getIncompleteWarning(parsed) : "",
    chooseFile,
    scan,
    save,
    applyDocumentType,
...
const numericFields = new Set([
  "gross", "net", "vat", "cash", "card", "salesCount",
  "openingCash", "expectedCash", "countedCash", "difference",
]);

function detectDocumentTypeRobust(text: string): ScanDocumentType {
  const direct = detectDocumentType(text);
  if (direct === "zReport") return direct;
  const normalized = text.toLowerCase();
  const hints = [
    /tages.?abschluss/, /z.?bericht/, /verkaufs.?ubersicht/, /zahlungs.?ubersicht/,
    /erwartetes.?bargeld/, /gezahlter.?bargeldbestand/, /anzahl.?verkaufe/,
  ];
  const score = hints.reduce((sum, pattern) => sum + (pattern.test(normalized) ? 1 : 0), 0);
  return score >= 2 ? "zReport" : "supplierInvoice";
}

function getIncompleteWarning(parsed: ParsedScan): string {
  if (parsed.type === "zReport") {
    return !parsed.gross && !parsed.cash && !parsed.card
